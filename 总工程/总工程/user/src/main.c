...
int main (void)
{
    ...
    encoder_quad_init(ENCODER_2, ENCODER_2_A, ENCODER_2_B);                     // 初始化编码器模块与引脚 正交解码编码器模式
		...
		PID_Init(&pid1,2.3f,0.9f,0.12f,170.0f);
		...
		PID_Init_line(&pidline,42.0f,0.0f,0.00f,99.0f);
		Kr=0.0;
		PID_SetOutputLimits_line(&pidline,-3000.0f,3000.0f);
		
		pit_ms_init(TIM8_PIT, 5);
    ...
    {//从最下面一行，访问到有效视野行
		...
		if(FLAG2==0)
		{
			if(FLAG3==1)
			{
				if(sum>0&&sum<7000)
				{
						Right_Add_Line(110,40,157,119);
				}
				
				if(sum>10102)
				{
					Left_Add_Line(150,40,40,119);
				}
				if(sum>=13600)
				{
					sum=0;
